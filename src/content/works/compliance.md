---
title: .COMpliance
slug: compliane
order: 3
year: 2026
materials_de: Domain, goldfarbener Notizblock mit eingebettetem Zugriffschip,
  E-Mail-Korrespondenz, Performance
materials_en: Domain, gold-colored notepad with embedded access chip, email
  correspondence, performance
dimensions_de: 11 x 6 x 3 cm
dimensions_en: 4 x 2 x 1 Inch
description_de: >-
  **.COMpliance**

  2026

  Domain, goldfarbener Notizblock mit eingebettetem Zugriffschip, E-Mail-Korrespondenz, Performance


  Ich besitze eine Domain, die niemand haben will. *[gallery-weekend-berlin.com](https://gallery-weekend-berlin.com)* liegt seit 2024 in meiner Hand. Ich habe sie dem Gallery Weekend Berlin angeboten. Kostenlos, ohne Bedingung. Statt Dank kam Vorbehalt. Fragen nach Konditionen, nach Absicherung. Eine Schenkung, für die es kein Protokoll gibt.


  Ich nenne das digitale Restitution. Der Begriff kommt aus der Debatte um geraubte Kunst. Vielleicht ist das zu groß für eine Domain. Aber die Verweigerung fühlte sich genauso an. Besitz wird verteidigt, auch wenn er niemandem nützt.


  Bis zur geplanten Übergabe blieb die Seite leer. Kein Programm, keine Werbung, nur ein Countdown, der auf die Rückgabe zählte.


  Den Zugriffscode habe ich in einen Chip eingebettet, verborgen in einem Notizblock, golden gefärbt, kein echtes Gold. Auch mein Gold ist nur Behauptung. Vielleicht war das die ganze Zeit die Übung: zu sehen, wessen Behauptung mehr zählt.


  Die ganze Korrespondenz habe ich veröffentlicht. Jede Mail, jeder Vorbehalt. Wer sie liest, sieht eine Institution, die auf Großzügigkeit nicht vorbereitet ist.


  Die Übergabe fand nicht statt. Auf meine Nachfragen kam keine Antwort mehr. Die Domain liegt weiter bei mir. Das Schweigen ist die einzige Antwort, die ich bekommen habe.
description_en: >-
  **.COMpliance**

  2026

  Domain, gold-colored notepad with embedded access chip, email correspondence, performance


  I own a domain that nobody wants. *[gallery-weekend-berlin.com](https://gallery-weekend-berlin.com)* has been in my hands since 2024. I offered it to Gallery Weekend Berlin. Free of charge, no conditions. Instead of thanks came reservation. Questions about terms, about safeguards. A gift with no protocol for how to receive it.


  I call this digital restitution. The term comes from the debate around looted art. Maybe that's too big a word for a domain. But the refusal felt exactly the same. Possession gets defended, even when it benefits no one.


  Until the planned handover, the site stayed empty. No program, no advertising, just a countdown ticking toward the return.


  I embedded the access code in a chip, hidden inside a notepad, painted gold, not real gold. My gold is only a claim too. Maybe that was the whole exercise: to see whose claim counts for more.


  I published the entire correspondence. Every email, every reservation. Whoever reads it sees an institution unprepared for generosity.


  The handover never happened. My follow-up messages went unanswered. The domain remains with me. The silence is the only answer I received.
cover_image: /uploads/compliance.jpg
gallery_images:
  - /uploads/compliance.jpg
  - /uploads/insta09.jpg
  - /uploads/compliance_sven_vollbrecht.jpg
---
